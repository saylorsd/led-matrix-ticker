...
class LEDMatrixTicker(object):
    ...
    def send_matrix_reg_byte(self,matrix, register, data):
        ...
        if matrix in self.matrices:
            padded_data = NO_OP * (len(self.matrices) - 1 - matrix) + [register, data] + NO_OP * matrix
    ...
    def send_all_reg_byte(self,register, data):
        ...
        self.send_bytes([register, data] * len(self.matrices))
    ...
    def scroll_message_horiz(self, message, repeats=0, speed=3, direction=DIR_L, font=DEFAULT_FONT, finish=True):
        """
    ...
    def scroll_one_text(self, message, delay, direction=DIR_L, font=None, pad=True):
        if pad:
            msg = (" " * self.width)
        else:
            msg = ""
        ...
        for c in range(len(msg) - (len(matrices)-1)):
            for stage in range(8):          # stage in the shift process
                for matrix in matrices:
                    old_char = msg[matrix + c]
                    try:
                        new_char = msg[matrix + c + 1]
                    except IndexError:
                        new_char = chr(0)
